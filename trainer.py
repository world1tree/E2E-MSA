...
class Trainer(object):
  ...
  def train(self, train_iter_fct, valid_iter_fct, train_steps, valid_steps):
    ...
    while step <= train_steps:
      ...
      for i, batch in enumerate(train_iter):
        # batch.src: [(seq_len_src, batch_size), batch_size] <eos>
        # batch.tgt: (seq_tgt_src, batch_size)               <bos> <eos>
        if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
          ...
          # tgt sentence是先有eos, 再有padding对齐的, <bos> nmu1, num2, ..., <eos>, <pad>, <pad> ...
          if self.norm_method == "tokens":
            # 这里num_tokens是算上了<eos>了，没有算上<bos>
  ...
  def _gradient_accumulation(self, true_batchs, normalization, total_stats,
      ...
      for batch in true_batchs:
          target_size = batch.tgt.size(0) # seq_len
